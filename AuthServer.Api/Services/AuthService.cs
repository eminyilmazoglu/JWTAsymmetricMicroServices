...
namespace AuthServer.Api.Services
{
    public class AuthService : IAuthService
    {
        ...
        {
            List<string> identityUserRoles = (List<string>)await _userManager.GetRolesAsync(identityUser);
            response.IsLogedIn = true;
            response.JwtToken = await this.GenerateTokenString(identityUser, identityUserRoles);
        ...
        private async Task<string> GenerateTokenString(IdentityUser identityUser, List<string> identityUserRoles)
        {
            var claims = new List<Claim>
            {
                new Claim(ClaimTypes.Name,identityUser.Email),
                new Claim(ClaimTypes.UserData,identityUser.Id) // opsionel...
            };

            foreach (string role in identityUserRoles)
            {
            ...
            ExtendedIdentityUser extendedIdentityUser = new ExtendedIdentityUser() { Id = identityUser.Id };
            await _userManager.SetAuthenticationTokenAsync(extendedIdentityUser, "AuthService", "loginToken - " + DateTime.Now.ToLongDateString(), tokenString);
